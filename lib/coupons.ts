...
export async function validCoupon(code:string,amount:number,memberId?:string|null,scope:'regular'|'special'='regular'):Promise<ManagedCoupon>{
 ...
 if((c.scope||'regular')!==scope)throw Error(scope==='special'?'รหัสนี้ไม่ใช่ CODE พิเศษ':'รหัสนี้ต้องกรอกในช่อง CODE พิเศษ');
